...
class RigidBody: Owner
{
    ...
    float damping = 0.8f;
    float dampingAngular = 0.8f;
    float stopThreshold = 0.1f; //0.15f
    float stopThresholdPV = 0.0f; //0.01f
    float stopThresholdAngular = 0.5f;
    float stopThresholdAngularPV = 0.0f;
    ...
    void integrateVelocities(float dt)
    {
        ...
        float da = clamp(1.0f - dt * dampingAngular, 0.0f, 1.0f);
        linearVelocity *= d;
        angularVelocity *= da;
        ...
        if (enableRotation)
        {
            if (angularVelocity.length < stopThresholdAngular) //stopThreshold
                angularVelocity = Vector3f(0.0f, 0.0f, 0.0f);
            else
            {
    ...
    void integratePseudoVelocities(float dt)
    {
        ...
        if (enableRotation)
        if (pseudoAngularVelocity.length > stopThresholdAngularPV)
        {
