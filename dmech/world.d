...
class PhysicsWorld: Owner
{
    ...
    DynamicArray!(BVHNode!Triangle) bvh;
        ...
        for (int i = 0; i < dynamicBodiesArray.length - 1; i++)
        {
            ...
            if (shape1.active)
            {
                for (int j = i + 1; j < dynamicBodiesArray.length; j++)
                {
                    ...
                    if (body2.active && (body1.collisionLayer == body2.collisionLayer || body1.collisionLayer == 0 || body2.collisionLayer == 0))
        ...
        foreach(body1; dynamicBodiesArray)
        {
            ...
            if (shape1.active)
            {
                foreach(body2; staticBodiesArray)
                {
                    if (body2.active)
                    foreach(shape2; body2.shapes.data)
                    if (shape2.active && (body1.collisionLayer == body2.collisionLayer || body1.collisionLayer == 0 || body2.collisionLayer == 0))
                    {
                        Contact c;
                        c.body1 = body1;
                        c.body2 = body2;
    ...
    void clearCollision(RigidBody b1, RigidBody b2)
    {
        foreach(shape1; b1.shapes.data)
        foreach(shape2; b2.shapes.data)
        {
