...
version(MemoryDebug)
{
    ...
    {
        records[cast(ulong)p] = AllocationRecord(type, size, counter, false, file, line);
        counter++;
    ...
    void printMemoryLog()
    {
        ...
        foreach(k; keys)
        {
            ...
            if (r.deleted)
                writefln("         %s - %s byte(s) in %s at line %s", r.type, r.size, k, r.file, r.line);
            else
                writefln("REMAINS: %s - %s byte(s) in %s at line %s", r.type, r.size, k, r.file, r.line);
        }
        writeln("----------------------------------------------------");
    ...
    void printAllocatedObjects()
    {
        ...
        foreach(k; keys)
        {
            AllocationRecord r = records[k];
            if (!r.deleted)
                writefln("%s - %s byte(s) allocated in %s(%s)", r.type, r.size, r.file, r.line);
        }
        writeln("----------------------------------------------------");
        writefln("Total amount of allocated memory: %s", _allocatedMemory);
        writeln("----------------------------------------------------");
